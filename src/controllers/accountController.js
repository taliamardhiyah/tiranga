...
const register = async (req, res) => {
    ...
    // Ambil nilai bonus dan uang nyata dari .env
    const bonusMoney = parseInt(process.env.BONUS_MONEY_ON_REGISTER || 0, 10);
    const realMoney = parseInt(process.env.REAL_MONEY_ON_REGISTER || 0, 10);
    ...
    if (username.length < 7 || username.length > 15 || !isNumber(username)) {
        return res.status(200).json({
            message: 'phone error',
            status: false
        });
    ...
    try {
        ...
        if (check_u.length == 1 && check_u[0].veri == 1) {
            return res.status(200).json({
                message: 'Nomor telepon terdaftar',
                status: false
            });
        } else {
            if (check_i.length == 1) {
                if (true) {
                    ...
                    // Tambahkan bonusMoney ke kolom money saat pendaftaran
                    ...
                    await connection.execute(sql, [id_user, username, name_user, md5(pwd), pwd, realMoney + bonusMoney, code, invitecode, ctv, 1, otp2, ip, 1, time]);
                    ...
                    return res.status(200).json({
                        message: "Telah berhasil terdaftar",
                        status: true
                    });
                } else {
                    return res.status(200).json({
...
const verifyCode = async (req, res) => {
    ...
    if (phone.length < 9 || phone.length > 13 || !isNumber(phone)) {
        return res.status(200).json({
            message: 'telepon salah: 81234567890',
            status: false
        });
    }
    ...
    if (rows.length == 0) {
        ...
        if (user.time_otp - now <= 0) {
            ...
            return res.status(200).json({
                message: 'Kirim SMS secara teratur',
                status: false,
                timeStamp: timeNow,
            });
        }
...
const forGotPassword = async (req, res) => {
    ...
    if (username.length < 9 || username.length > 13 || !isNumber(username)) {
        return res.status(200).json({
            message: 'Nomor telepon salah',
            status: false
        });
