...
const dicePage = async (req, res) => {
  try {
    const [rows] = await connection.query(`SELECT dice.*, users.name_user FROM dice LEFT JOIN users ON dice.user_id = users.id_user ORDER BY dice.id DESC LIMIT 50`);
    return res.render("admin/dicePage.ejs", { rows });
  } catch (error) {
    console.error(error);
    return res.status(500).send("Error loading dice admin page");
  }
};
