...
const add5D = async(game) => {
    try {
        ...
        // Log query SQL
        const selectPeriodQuery = `SELECT period FROM 5d WHERE status = 0 AND game = ${game} ORDER BY id DESC LIMIT 1`;
        console.log('Executing SQL Query:', selectPeriodQuery);
        
        let [k5D] = await connection.query(selectPeriodQuery);
        const [setting] = await connection.query('SELECT * FROM `admin` ');

        // Tambahkan log ini untuk memastikan nilai k5D
        console.log('k5D:', k5D);

        // Tambahkan pengecekan untuk memastikan k5D tidak kosong
        if (k5D.length === 0) {
            throw new Error('No active period found for the game');
        }
