...
import { depositJayapay } from '../controllers/paymentController';
// Tambahkan di routes/index.js atau routes/redblack.js
import {
  redBlackPage,
  betRedBlack,
  getRedBlackStatus,
  getUserSaldo
} from "../controllers/redBlackController.js";
// filepath: d:\xampp\htdocs\786lottery\786lottery\src\routes\redBlackRoutes.js
import { getLeaderboardRedBlack } from '../controllers/redBlackController.js';
...
const initWebRouter = (app) => {
    ...
    // promo
    router.get('/promo/bonus', (req, res) => {
    res.render('promo/bonus');
});

const paymentCtrl = require('../controllers/paymentController');

router.get('/deposit/jayapay', paymentCtrl.depositJayapayForm);
router.post('/deposit/jayapay', paymentCtrl.depositJayapay);
router.post('/jayapay/callback', paymentCtrl.jayapayCallback);

    // game kartu
    router.get('/game/redblack', redBlackPage);
    router.post('/api/redblack/bet', betRedBlack);
    router.get('/api/redblack/status', getRedBlackStatus);
    router.get('/api/user/saldo', getUserSaldo);
    // ...
router.get('/redblack/leaderboard', getLeaderboardRedBlack);

//app.post('/deposit/jayapay', depositJayapay);
router.get('/deposit/jayapay', depositJayapay);
router.get('/wallet/deposit-jayapay', (req, res) => {
  res.json({status: "OK", message: "Gunakan POST request untuk deposit Jayapay"});
});
// ROUTE HALAMAN DICE
router.get("/dice", diceController.index); // ✅ sesuai dengan method yang ada
router.post("/api/dice/play", diceController.play);  // untuk main dice

// <!-- Tambah di menu sidebar atau navbar -->
// <li><a href="/dice">🎲 Dice Game</a></li>

// Route Winstrike WINGO
router.get('/activity/winstrike', (req, res) => {
  res.render('winstrike');
});
